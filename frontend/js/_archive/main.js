...
self.__LGMDM_BUILD__ = '1.6';
console.log('%c[LGMDM] build 1.6', 'color:#08d;font-weight:bold;background:#000;padding:2px 6px;border-radius:3px');
...
import { openConfirmPanel } from './confirm-panel.js';
...
bindOnce($('btn-master'), 'click', async () => {
  ...
  // BUGFIX Sep 12: mostrar resumen del chain activo + parámetros antes de
  // disparar el job. Si el usuario cancela, no se hace nada.
  const ok = await openConfirmPanel({ file: file_, title: 'Confirmar masterización' });
  if (!ok) return;
...
bindOnce($('btn-auto-master'), 'click', async () => {
  const file_ = state.get('selectedFile');
  if (!file_) { showProgress('error'); return; }
  // BUGFIX Sep 12: panel de confirmación también para auto-master. La IA
  // decidirá los parámetros server-side, pero el usuario debe confirmar.
  const ok = await openConfirmPanel({ file: file_, title: 'Confirmar auto-mastering (IA)' });
  if (!ok) return;
  resultsArea.innerHTML = '';
