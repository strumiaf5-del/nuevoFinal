...
function drawFFTOnCanvas(canvas, series) {
  ...
  series.forEach((s) => {
    ...
    ctx.lineWidth = 2;
    freqs.forEach((f, i) => {
      const x = window.xFromFreq(Math.max(f, fMin), padL, plotW, Math.log10(fMin), Math.log10(fMax));
...
function perceptualPanelHtml(a, titleSuffix) {
  ...
  const rows = Object.entries(PERCEPTUAL_LABELS)
    .map(([key, label]) => {
      ...
      const text = window.LGMDM.ui.escapeHtml(String(val).replace(/_/g, " "));
      return `<div class="metric-row"><span class="metric-label">${window.LGMDM.ui.escapeHtml(label)}</span><span class="metric-value ${cls}">${text}</span></div>`;
    })
    .join("");
  ...
  return `
    <div class="analysis-panel perceptual-panel">
      ...
      ${genre ? `<div class="perceptual-genre"><span class="perceptual-genre-tag">${window.LGMDM.ui.escapeHtml(genre)}</span>${genreConf != null ? `<span class="perceptual-genre-conf">${window.LGMDM.ui.escapeHtml(genreConf)}% confianza</span>` : ""}</div>` : ""}
      ${diagnosis ? `<p class="perceptual-diagnosis">${window.LGMDM.ui.escapeHtml(diagnosis)}</p>` : ""}
...
function _renderABPlayer() {
  ...
  _abUiTimer = setInterval(() => {
    if (window.LGMDM.utils.prefersReducedMotion()) return;
...
async function setupABPlayer(masterBlob) {
  try {
    const cachedBuf = window.LGMDM?.state?.cachedFileBuffer;
    const origBlob = cachedBuf
      ? new Blob([cachedBuf], { type: window.LGMDM.state.selectedFile?.type || "audio/wav" })
      : null;
    if (!origBlob) return;
...
window.renderAnalysisSingle = renderAnalysisSingle;
window.renderAnalysisComparison = renderAnalysisComparison;
window.renderPerceptualStandalone = renderPerceptualStandalone;
