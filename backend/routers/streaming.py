...
def create_router(**dependencies):
    ...
    global validate_audio_file, verify_ws_token, extract_ws_token, WS_SUBPROTOCOL, analyze_audio
    ...
    extract_ws_token = dependencies["extract_ws_token"]
    WS_SUBPROTOCOL = dependencies["WS_SUBPROTOCOL"]
...
async def ws_master_stream(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json({"event":"error","message":"Error interno en el stream"})
    finally:
...
async def mix_submit(
    ...
    try:
        ...
        logger.error("JSON inválido en /mix/submit: %s", e, exc_info=True)
        raise HTTPException(400, "JSON inválido: operación no completada")
...
async def ws_mix_stream(websocket: WebSocket):
    # El frontend pide un ticket corto a /auth/ws-ticket y lo manda como
    # subprotocol "lgmdm-ws-ticket.<token>" (o, en compatibilidad, ?token=).
