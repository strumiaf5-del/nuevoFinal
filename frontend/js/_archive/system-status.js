...
export async function startSystemStatus() {
  ...
  // BUGFIX Sep 12: apiBase() puede ser absoluto (otro subdominio como en
  // producción) o relativo ("/api"/vacío). Antes siempre usaba location.host,
  // así que cuando apiBase era "https://masteringstudio-api.duckdns.org" el WS
  // apuntaba a "wss://masteringstudio.duckdns.org/ws/dashboard..." → CSP/CORS
  // reject y fallback HTTP se llevaba el flood de /dashboard cada 2s.
  let wsUrl;
  const api = apiBase();
  if (api.startsWith('http://') || api.startsWith('https://')) {
    const u = new URL(api);
    wsUrl = `${u.protocol === 'https:' ? 'wss:' : 'ws:'}//${u.host}/ws/dashboard?token=${encodeURIComponent(ticket)}`;
  } else {
    const host = location.host;
    const basePath = api || '';
    wsUrl = `${proto}://${host}${basePath}/ws/dashboard?token=${encodeURIComponent(ticket)}`;
  }
  const url = wsUrl;
