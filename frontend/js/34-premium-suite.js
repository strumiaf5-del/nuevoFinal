...
(function (global) {
  ...
  // ── AUDIO TAP — extraído a 35-audio-tap.js ──────────────────────────────
  // Acceso vía LGMDM.proFeatures.audioTap.ensure() / .teardown() / .bandSpecs
  ...
  // _WATERFALL_LUT, colorForMagnitude, drawGoniometerFrame, drawWaterfallFrame
  // movidos a pro-features/visualizer-render.js (split JS-I-3).
  // Consumimos via window.LGMDM.visualizerRender.* — ver visualizer-render.js.
  const drawGoniometerFrame = window.LGMDM?.visualizerRender?.drawGoniometerFrame;
  const drawWaterfallFrame = window.LGMDM?.visualizerRender?.drawWaterfallFrame;
  const colorForMagnitude = window.LGMDM?.visualizerRender?.colorForMagnitude;
    ...
    modal.innerHTML = `
      <div class="pro-dialog pro-dialog-shell">
        ...
        <div class="pro-tab-header">
          <button class="pro-tab-btn active" data-premium-tab="compliance"><span aria-hidden="true">📋</span> 1. Compliance</button>
          <button class="pro-tab-btn" data-premium-tab="abx"><span aria-hidden="true">⚖️</span> 2. Blind ABX</button>
          <button class="pro-tab-btn" data-premium-tab="stereo"><span aria-hidden="true">📐</span> 3. Stereo & Mono</button>
          <button class="pro-tab-btn" data-premium-tab="codec"><span aria-hidden="true">🎧</span> 4. Codecs</button>
          <button class="pro-tab-btn" data-premium-tab="waterfall"><span aria-hidden="true">🌊</span> 5. Waterfall 3D</button>
          <button class="pro-tab-btn" data-premium-tab="doctor"><span aria-hidden="true">🩺</span> 6. AI Doctor</button>
          <button class="pro-tab-btn" data-premium-tab="demask"><span aria-hidden="true">🎛</span> 7. Demasking</button>
          <button class="pro-tab-btn" data-premium-tab="tamer"><span aria-hidden="true">🎯</span> 8. Resonance Tamer</button>
          <button class="pro-tab-btn" data-premium-tab="warmer"><span aria-hidden="true">🔥</span> 9. Vintage Warmer</button>
          <button class="pro-tab-btn" data-premium-tab="matcheq"><span aria-hidden="true">📊</span> 10. Match EQ</button>
          <button class="pro-tab-btn" data-premium-tab="phantomsub"><span aria-hidden="true">🔊</span> 11. Phantom Sub</button>
          <button class="pro-tab-btn" data-premium-tab="stemsep"><span aria-hidden="true">🪄</span> 12. AI Stem Separator</button>
          <button class="pro-tab-btn" data-premium-tab="loudness-penalty"><span aria-hidden="true">🎯</span> 13. Loudness Penalty</button>
          <button class="pro-tab-btn" data-premium-tab="spectral-tilt"><span aria-hidden="true">📈</span> 14. Spectral Tilt</button>
          <button class="pro-tab-btn" data-premium-tab="multiband-transient"><span aria-hidden="true">🥁</span> 15. Multiband Transient</button>
          <button class="pro-tab-btn" data-premium-tab="ms-imager"><span aria-hidden="true">📐</span> 16. M/S Imager</button>
          <button class="pro-tab-btn" data-premium-tab="reference-match"><span aria-hidden="true">🎚</span> 17. Reference Match</button>
          <button class="pro-tab-btn" data-premium-tab="dr-meter"><span aria-hidden="true">📊</span> 18. DR Meter</button>
          <button class="pro-tab-btn" data-premium-tab="saturation"><span aria-hidden="true">🔥</span> 19. Saturation</button>
          <button class="pro-tab-btn" data-premium-tab="phase-rotation"><span aria-hidden="true">🔄</span> 20. Phase Rotation</button>
          <button class="pro-tab-btn" data-premium-tab="reverb"><span aria-hidden="true">🌫</span> 21. Reverb</button>
          <button class="pro-tab-btn" data-premium-tab="loudness-war"><span aria-hidden="true">📉</span> 22. Loudness War</button>
          <button class="pro-tab-btn" data-premium-tab="iso-compensation"><span aria-hidden="true">🔉</span> 23. ISO 226 Compensation</button>
        </div>
  ...
  function renderActiveTab() {
    ...
    const tab = (key) => LGMDM?.proFeatures?.tabs?.[key] || globalThis[`_lgmdmRender${key.charAt(0).toUpperCase() + key.slice(1)}Tab`];
    const TAB_RENDERERS = {
      'compliance':       (c) => (tab('compliance') || renderComplianceTab)(c, { lufs, tp, lra, isLive }),
      'abx':              (c) => (tab('abx') || renderAbxTab)(c),
      'stereo':           (c) => renderStereoTab(c),
      'codec':            (c) => (tab('codec') || renderCodecTab)(c),
      'waterfall':        (c) => (tab('waterfall') || renderWaterfallTab)(c),
      ...
      'spectral-tilt':    (c) => renderSpectralTiltTab(c),
      'multiband-transient': (c) => renderMultibandTransientTab(c),
      'ms-imager':        (c) => renderMsImagerTab(c),
      'reference-match':  (c) => renderReferenceMatchTab(c),
      'dr-meter':         (c) => renderDrMeterTab(c),
      'saturation':       (c) => renderSaturationTab(c),
      'phase-rotation':   (c) => renderPhaseRotationTab(c),
      'reverb':           (c) => renderReverbTab(c),
      'loudness-war':     (c) => renderLoudnessWarTab(c),
      'iso-compensation': (c) => renderIsoCompensationTab(c),
    };
  ...
  // F5.2 — Cleanup delegado a LGMDM.proFeatures.audioTap.teardown (35-audio-tap.js).
