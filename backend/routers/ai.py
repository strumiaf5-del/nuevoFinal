...
def _safe_filename(name: str | None, fallback_prefix: str = "file") -> str:
    """Sanitiza un nombre de archivo para uso decorativo en paths internos.

    - Toma solo el basename (descarta separadores de directorio).
    - Reemplaza cualquier caracter fuera de [A-Za-z0-9._-] por '_'.
    - Limita la longitud para evitar nombres absurdamente largos.
    - Si el resultado queda vacío, genera un nombre basado en uuid.
    """
    if not name:
    ...
    if len(base) > 96:
        ...
        if dot and 0 < len(ext) <= 8:
            base = stem[: 96 - len(ext) - 1].rstrip("._-") + "." + ext
        else:
            base = base[:96]
    return base
...
def create_ai_router(*, upload_dir: str, read_and_validate, resolve_input_source, validate_audio_file, jobs, logger, run_mastering_job, current_user_dependency, limiter) -> APIRouter:
    ...
    async def ai_suggest(request: Request,
        ...
        safe_name = _safe_filename(filename, fallback_prefix="aisuggest")
        tmp_path = os.path.join(upload_dir, f"aisuggest_{uuid.uuid4().hex}_{safe_name}")
    ...
    async def ai_auto_master(request: Request,
        ...
        safe_name = _safe_filename(file.filename, fallback_prefix=job_id)
