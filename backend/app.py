...
WS_SUBPROTOCOL = "lgmdm-ws-ticket"


def extract_ws_token(websocket: WebSocket) -> Optional[str]:
    """Extrae el ticket de WebSocket primero del header Sec-WebSocket-Protocol
    y, si no está, cae al query param ?token= (compatibilidad hacia atrás).

    El cliente moderno envía dos subprotocolos: ["lgmdm-ws-ticket", "<token>"].
    El servidor hace eco de "lgmdm-ws-ticket" en accept(subprotocol=...) y
    extrae el token del segundo slot. El token NUNCA aparece en la URL, así
    no queda en logs de proxies ni en el history del browser.

    Formatos aceptados del header (orden de preferencia):
      1) "lgmdm-ws-ticket, <token>"  → array (canónico)
      2) "lgmdm-ws-ticket.<token>"   → prefijo pegado (legacy-friendly)
      3) "?token=<token>"            → fallback de query string

    Si el header NO contiene el marker "lgmdm-ws-ticket" (ni como slot ni
    como prefijo pegado), se ignora y se cae al query param — evita aceptar
    tokens de clientes que negocian otros protocolos.
    """
    raw = websocket.headers.get("sec-websocket-protocol") or ""
    slots = [p.strip() for p in raw.split(",") if p.strip()]
    marker_present = any(
        s == WS_SUBPROTOCOL or s.startswith(WS_SUBPROTOCOL + ".")
        for s in slots
    )
    if marker_present:
        # Regla: el primer slot que NO sea el marker exacto es el token;
        # si empieza con el marker pegado, le quitamos el prefijo.
        for name in slots:
            if name == WS_SUBPROTOCOL:
                continue
            if name.startswith(WS_SUBPROTOCOL + "."):
                suffix = name[len(WS_SUBPROTOCOL) + 1:].strip()
                if suffix:
                    return suffix
                continue
            return name
...
# D-3: lifespan — graceful shutdown: registrar arranque, luego en shutdown
# esperar (máx 30s) a los jobs activos antes de cerrar el proceso.
@asynccontextmanager
async def lifespan(app: FastAPI):
    global START_TIME
    START_TIME = time.time()
    app.state.START_TIME = START_TIME
    logger.info("LGMDM startup: registrando START_TIME=%.3f", START_TIME)
    yield
    # D-3: shutdown — esperar jobs activos (máx 30s) antes de morir
    logger.info("LGMDM shutdown: esperando jobs activos (timeout 30s) ...")
    try:
        running_jobs = [j for j in jobs.get_all().values() if j.get("status") == "processing"]
        if running_jobs:
            logger.info("LGMDM shutdown: %d job(s) activo(s) en curso", len(running_jobs))
        for job in running_jobs:
            try:
                await asyncio.wait_for(job.wait(), timeout=30)
            except (asyncio.TimeoutError, AttributeError, TypeError) as exc:
                logger.warning("LGMDM shutdown: job %s no finalizó en 30s (%s)", job.get("job_id", "?"), type(exc).__name__)
    except Exception:
        logger.exception("LGMDM shutdown: error al esperar jobs activos")
    logger.info("LGMDM Graceful shutdown complete")

app = FastAPI(title="Audio Mastering API", version="7.0.1", lifespan=lifespan)
...
# ── MAX_FILE_SIZE DoS protection ────────────────────────────────────────────
# Starlette procesa middlewares en orden INVERSO al que se agregan: este se
# registra DESPUÉS de CORSMiddleware, así que corre ANTES que CORS y rechaza
# con 413 ANTES de leer el body (content-length está disponible sin leer el
# stream). Es defensa contra clientes que mandan archivos >MAX_FILE_SIZE
# (default 300MB) para agotar RAM via `await file.read()` en routers/dsp/*
...
class MaxFileSizeMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        if cl and cl.isdigit() and int(cl) > MAX_FILE_SIZE:
            return JSONResponse(
                status_code=413,
                content={"detail": f"Archivo excede MAX_FILE_SIZE ({MAX_FILE_SIZE // (1024*1024)} MB)"},
            )
        return await call_next(request)

app.add_middleware(MaxFileSizeMiddleware)

# ── Security HTTP headers (B-S1) ───────────────────────────────────────────
# Endurece toda respuesta con headers de seguridad estándar. Se registra
# DESPUÉS de MaxFileSize/CORS, así que es el middleware MÁS externo sobre la
# respuesta (corre último al añadir headers). Uso `set` para asegurar el
# header aunque una respuesta interna ya intente bajarlo; no toco ACAO (CORS
# se maneja aparte en enforce_frontend_cors).
# CSP elegida: `default-src 'self'; style-src 'self' 'unsafe-inline'; img-src
# 'self' data:`. La app sirve el frontend como archivo plano SIN bundler y
# usa estilos inline + data URIs de forma intensiva; un CSP puro `default-src
# 'self'` rompería el layout y los spectrometers del canvas. `unsafe-inline`
# se limita al scope `style-src` (no a `script-src`), así que el JS sigue
# confinado a origen propio. Documentado en AGENTS.md.
...
# Exception handler global: cuando una excepción no atrapada ocurre durante
# la serialización del response (p.ej. jsonable_encoder sobre un job con
# params no serializables), Starlette retorna un 500 plano SIN headers CORS.
# El navegador entonces bloquea la respuesta por CORS y el frontend no puede
# leer el error. Este handler intercepta TODAS las excepciones y devuelve un
# JSONResponse con los headers CORS correctos para que el frontend siempre
...
async def _cors_aware_500_handler(request: Request, exc: Exception):
    ...
    if origin in CORS_ORIGINS:
        ...
        headers["Vary"] = "Origin"
    # HTTPException ya tiene su propio handler en Starlette; solo capturamos
    # excepciones genuinamente no manejadas.
    status = getattr(exc, "status_code", 500)
    # Antes: detail = getattr(exc, "detail", str(exc)) filtraba stack traces
    # al cliente. Ahora: usar mensaje genérico para no-HTTPException; el
    # logger.exception de arriba retiene la traza interna completa.
...
app.include_router(create_dashboard_router(
    ...
    extract_ws_token=extract_ws_token,
...
app.include_router(create_auth_router(logger=logger, limiter=limiter))
...
app.include_router(create_advanced_dsp_router(
    upload_dir=UPLOAD_DIR,
    processed_dir=PROCESSED_DIR,
    get_current_user=get_current_user,
    logger=logger
