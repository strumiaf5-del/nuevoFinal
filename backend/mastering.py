...
class MasteringParams:
    ...
    # ── Advanced DSP Suite ───────────────────────────────────────────────────
    adv_tamer_bypass: bool = True
    adv_tamer_sensitivity: float = 0.5
    adv_tamer_depth_db: float = -6.0
    adv_tamer_bands: int = 64
    adv_inflator_bypass: bool = True
    adv_inflator_drive: float = 0.5
    adv_inflator_mix: float = 1.0
    adv_inflator_curve: str = "chebyshev"
    adv_phantom_sub_bypass: bool = True
    adv_phantom_sub_crossover: float = 80.0
    adv_phantom_sub_mix: float = 0.5
    adv_phantom_sub_mode: str = "fifth"
    adv_iso_bypass: bool = True
    adv_iso_playback_phon: float = 40.0
    adv_iso_strength: float = 0.5
...
def _crop_preview(audio: np.ndarray, sr: int, preview_seconds: float, start_sec: float = 15.0) -> np.ndarray:
    """Recorta `preview_seconds` segundos a partir de `start_sec`.

    Default start_sec=15 mantiene compatibilidad con el comportamiento previo
    (el usuario reportó que si elegía empezar en 35s seguía arrancando en 0).
    Si el archivo es más corto, se corre el inicio hacia atrás para no perder
    material al final.
    """
    ...
    last_valid_start = max(0, total_samples - max_samples)
    start = min(int(start_sec * sr), last_valid_start)
    cropped = audio[:, start:start + max_samples].copy()
    fade_in_len = min(int(0.005 * sr), max_samples // 4)
    fade_out_len = min(int(0.010 * sr), max_samples // 4)
    if fade_in_len > 0:
        t = np.linspace(0.0, 1.0, fade_in_len, dtype=cropped.dtype)
        cropped[:, :fade_in_len] *= t
    if fade_out_len > 0:
        t = np.linspace(1.0, 0.0, fade_out_len, dtype=cropped.dtype)
        cropped[:, -fade_out_len:] *= t
    return cropped
...
def _prepare_pyln_data(audio: np.ndarray) -> np.ndarray:
    """Normaliza audio de entrada para pyloudnorm / BS.1770 con shape (samples, channels)."""
    if audio.ndim == 1:
        return audio.reshape(-1, 1).astype(np.float64)
    elif audio.ndim == 2:
...
def filter_k_weighting(audio: np.ndarray, sr: int) -> np.ndarray:
    """Aplica el filtro K-weighting de dos etapas (Stage 1 High-Shelf + Stage 2 High-Pass)
    definido en ITU-R BS.1770-4 a cada canal de audio.
    Devuelve un array con forma (samples, channels).
    """
    import pyloudnorm as pyln
    data = _prepare_pyln_data(audio)
    filtered = np.zeros_like(data, dtype=np.float64)
    meter = pyln.Meter(sr)
    for ch in range(data.shape[1]):
        sig = data[:, ch].astype(np.float64)
        for stage in meter._filters.values():
            sig = stage.apply_filter(sig)
        filtered[:, ch] = sig
    return filtered


def _channel_weights(num_channels: int) -> list[float]:
    """Factores de ganancia por canal (G_i) según ITU-R BS.1770-4 Tabla 1.
    5.1 surround layout (L, R, C, LFE, Ls, Rs):
    LFE (canal 4, índice 3) tiene ganancia 0.0 (excluido del cálculo).
    Surround Ls y Rs (canales 5 y 6, índices 4 y 5) tienen factor 1.41 (~+1.5 dB).
    """
    if num_channels == 1:
        return [1.0]
    elif num_channels == 2:
        ...
        return [1.0, 1.0, 1.0, 0.0, 1.41, 1.41] + [1.0] * (num_channels - 6)
    return [1.0] * num_channels


def _k_weighted_power(filtered: np.ndarray) -> np.ndarray:
    """Calcula la potencia instantánea ponderada por canal según ITU-R BS.1770-4."""
    num_channels = filtered.shape[1]
    weights = _channel_weights(num_channels)
    power = np.zeros(filtered.shape[0], dtype=np.float64)
    for ch in range(num_channels):
        w = weights[ch] if ch < len(weights) else 1.0
        power += w * (filtered[:, ch] ** 2)
    return power


def _sliding_window_loudness(power: np.ndarray, window_samples: int, hop_samples: int) -> np.ndarray:
    """Calcula la serie temporal de loudness (LUFS) sobre ventanas rectangulares deslizantes."""
    n_samples = len(power)
    if n_samples == 0:
    ...
    if n_samples < window_samples:
        ...
        if mean_p <= 1e-12:
            return np.array([-70.0])
        return np.array([-0.691 + 10.0 * np.log10(mean_p)])

    cumsum = np.concatenate([[0.0], np.cumsum(power, dtype=np.float64)])
    block_sums = cumsum[window_samples:] - cumsum[:-window_samples]
    sampled = block_sums[::hop_samples] / window_samples
    sampled = np.maximum(sampled, 1e-12)
    loudness = -0.691 + 10.0 * np.log10(sampled)
    return loudness


def measure_momentary_lufs(audio: np.ndarray, sr: int, hop_s: float = 0.1, return_series: bool = False):
    """Calcula el Loudness Momentáneo (M) con ventana rectangular de 400ms según ITU-R BS.1770-4 / EBU R128.
    Retorna el máximo observado (`momentary_max_lufs`), o `(series, max)` si `return_series=True`.
    """
    filtered = filter_k_weighting(audio, sr)
    power = _k_weighted_power(filtered)
    window_samples = max(1, int(round(0.4 * sr)))
    hop_samples = max(1, int(round(hop_s * sr)))
    series = _sliding_window_loudness(power, window_samples, hop_samples)
    max_val = float(np.max(series)) if len(series) > 0 else -70.0
    if return_series:
        return series, max_val
    return max_val


def measure_short_term_lufs(audio: np.ndarray, sr: int, hop_s: float = 0.1, return_series: bool = False):
    """Calcula el Loudness de Corto Plazo (S) con ventana rectangular de 3.0s según ITU-R BS.1770-4 / EBU R128.
    Retorna el máximo observado (`short_term_max_lufs`), o `(series, max)` si `return_series=True`.
    """
    filtered = filter_k_weighting(audio, sr)
    power = _k_weighted_power(filtered)
    window_samples = max(1, int(round(3.0 * sr)))
    hop_samples = max(1, int(round(hop_s * sr)))
    series = _sliding_window_loudness(power, window_samples, hop_samples)
    max_val = float(np.max(series)) if len(series) > 0 else -70.0
    if return_series:
...
def momentary_and_short_term_lufs(audio: np.ndarray, sr: int, hop_s: float = 0.1) -> tuple[float, float]:
    """Calcula `momentary_max_lufs` y `short_term_max_lufs` en una única pasada de filtrado K-weighting."""
    filtered = filter_k_weighting(audio, sr)
    power = _k_weighted_power(filtered)
    hop_samples = max(1, int(round(hop_s * sr)))

    m_win = max(1, int(round(0.4 * sr)))
    m_series = _sliding_window_loudness(power, m_win, hop_samples)
    m_max = float(np.max(m_series)) if len(m_series) > 0 else -70.0

    s_win = max(1, int(round(3.0 * sr)))
    s_series = _sliding_window_loudness(power, s_win, hop_samples)
    s_max = float(np.max(s_series)) if len(s_series) > 0 else -70.0

    return m_max, s_max


def compute_meters_timeline(audio: np.ndarray, sr: int, block_ms: float = 33.0) -> dict:
    """Compute per-block meters timeline for real-time display (100% server-side).

    Returns a dict with:
      - timeline: list of {t_ms, peak_db, rms_db, lufs, stereo_corr, spectrum: [6]}
      - hop_ms: block size in ms
      - n_blocks: number of blocks

    Spectrum bands match the HTML labels:
    ...
    if n_samples == 0 or sr <= 0:
        return {"timeline": [], "hop_ms": block_ms, "n_blocks": 0}

    block_samples = max(1, int(round(block_ms / 1000.0 * sr)))
    n_blocks = max(1, int(np.ceil(n_samples / block_samples)))
    hop_ms = round(1000.0 * block_samples / sr, 2)

    # Pad to exact multiple of block_samples for vectorized reshape
    pad_len = n_blocks * block_samples - n_samples
    if pad_len > 0:
        audio_padded = np.pad(audio, ((0, 0), (0, pad_len)), mode='constant')
    else:
        audio_padded = audio

    # Reshape to (channels, n_blocks, block_samples) for vectorized ops
    reshaped = audio_padded.reshape(audio.shape[0], n_blocks, block_samples)

    # Peak per block (max abs across channels and samples)
    peak_lin = np.max(np.abs(reshaped), axis=(0, 2))
    peak_db = 20.0 * np.log10(peak_lin + 1e-12)

    # RMS per block
    rms = np.sqrt(np.mean(reshaped ** 2, axis=(0, 2)))
    rms_db = 20.0 * np.log10(rms + 1e-12)

    # Stereo correlation per block
    if reshaped.shape[0] >= 2:
        l = reshaped[0]
        r = reshaped[1]
        sum_lr = np.sum(l * r, axis=1)
        sum_l2 = np.sum(l ** 2, axis=1)
        sum_r2 = np.sum(r ** 2, axis=1)
        denom = np.sqrt(sum_l2 * sum_r2)
        corr = np.where(denom > 1e-12, sum_lr / denom, 0.0)
    else:
        corr = np.ones(n_blocks)

    # LUFS momentary per block (400ms window, block_ms hop)
    try:
        lufs_series, _ = measure_momentary_lufs(
            audio, sr, hop_s=block_ms / 1000.0, return_series=True
        )
        if len(lufs_series) < n_blocks:
            pad = np.full(n_blocks - len(lufs_series), -70.0)
            lufs_series = np.concatenate([pad, lufs_series])
        elif len(lufs_series) > n_blocks:
            lufs_series = lufs_series[:n_blocks]
    except Exception:
        lufs_series = np.full(n_blocks, -70.0)

    # Spectrum per block (6 bands matching HTML labels)
    band_edges = [
        (20, 60), (60, 250), (250, 800),
        (800, 3000), (3000, 8000), (8000, 20000),
    ]
    spectrum_blocks = np.full((n_blocks, 6), -120.0)
    try:
        from scipy.signal import stft as _stft
        mono = np.mean(audio_padded, axis=0) if audio_padded.shape[0] > 1 else audio_padded[0]
        nperseg = min(2048, block_samples * 4, n_samples)
        if nperseg < 256:
            nperseg = 256
        noverlap = max(0, nperseg - block_samples)
        f, t_stft, Zxx = _stft(
            mono, fs=sr, nperseg=nperseg, noverlap=noverlap, boundary=None, padded=False
        )
        mag_db = 20.0 * np.log10(np.abs(Zxx) + 1e-12)
        n_stft_frames = min(mag_db.shape[1], n_blocks)
        for b, (f_lo, f_hi) in enumerate(band_edges):
            band_mask = (f >= f_lo) & (f < f_hi)
            if np.any(band_mask):
                band_vals = mag_db[band_mask, :n_stft_frames]
                band_mean = np.mean(band_vals, axis=0)
                spectrum_blocks[:n_stft_frames, b] = band_mean
                if n_stft_frames < n_blocks:
                    spectrum_blocks[n_stft_frames:, b] = band_mean[-1] if len(band_mean) > 0 else -120.0
    except Exception:
        pass
    ...
    for i in range(n_blocks):
        timeline.append({
            "t_ms": round(i * hop_ms, 1),
            "peak_db": round(float(peak_db[i]), 2),
            "rms_db": round(float(rms_db[i]), 2),
            "lufs": round(float(lufs_series[i]), 2),
            "stereo_corr": round(float(corr[i]), 3),
            "spectrum": [round(float(v), 2) for v in spectrum_blocks[i]],
        })

    return {"timeline": timeline, "hop_ms": hop_ms, "n_blocks": n_blocks}


momentary_max_lufs = measure_momentary_lufs
short_term_max_lufs = measure_short_term_lufs
measure_momentary_max_lufs = measure_momentary_lufs
measure_short_term_max_lufs = measure_short_term_lufs
...
def measure_lufs_integrated(audio: np.ndarray, sr: int) -> float:
    try:
        ...
        if data.shape[1] <= 5:
            meter = pyln.Meter(sr)
            if len(data) >= int(0.4 * sr):
                val = meter.integrated_loudness(data)
                if np.isfinite(val):
                    return float(val)
            else:
                filtered = filter_k_weighting(data, sr)
                power = _k_weighted_power(filtered)
    ...
    try:
        # Normalizar forma a canales-primero para audio multicanal (> 5 canales, ej. 5.1 surround)
        if audio.ndim == 2 and audio.shape[0] > audio.shape[1] and audio.shape[1] <= 16:
            ch_data = audio.T
        else:
            ch_data = audio if audio.ndim == 2 else audio[np.newaxis, :]
        filtered = filter_k_weighting(ch_data.T, sr)
        weights = _channel_weights(len(ch_data))
        power = np.zeros(filtered.shape[0], dtype=np.float64)
...
measure_integrated_lufs = measure_lufs_integrated
...
def linear_phase_eq(audio: np.ndarray, sr: int, bands: list, num_taps: int = None) -> np.ndarray:
    """EQ de FASE LINEAL real (FIR). num_taps adaptativo si es None.

    Antes: default fijo num_taps=2049 → freq resolution ~21 Hz @ 44.1kHz, insuficiente
    para bandas <50 Hz. Ahora: num_taps adaptativo según sr; usa 4097 por default
    para garantizar <11 Hz de resolución y cubrir la banda log más baja (25 Hz).
    """
    if num_taps is None:
        # Resolución objetivo: < 11 Hz a sr típico. 4097 taps cubre hasta 96 kHz.
        # Usar impar para que firwin2 retorne filtro tipo I (linear phase).
        num_taps = 4097
    elif num_taps % 2 == 0:
        num_taps += 1
...
def detect_sibilance(audio: np.ndarray, sr: int,
    ...
    if len(mono) < 16 or high_hz <= low_hz:
        return {
            "sibilance_ratio": 0.0,
            "is_sibilant": False,
            "reduction_db": 0.0,
    ...
    return {
        "sibilance_ratio": round(float(severity), 2),
        "is_sibilant": present,
        "reduction_db": round(float(min(severity, 8.0)), 2),
        "present": present,
        "band_hz": [round(low_hz, 1), round(high_hz, 1)],
...
def measure_lra(audio: np.ndarray, sr: int, block_s: float = 3.0, hop_s: float = 1.0) -> float:
    """Calcula el Loudness Range (LRA) según EBU Tech 3342 usando pyloudnorm.
    Maneja de forma robusta audios cortos (< 3.0s), silencios o excepciones
    para nunca crashear ni devolver valores negativos.
    """
    try:
        import pyloudnorm as pyln
        # F2.4: silencio puro (RMS=0) hace que pyloudnorm devuelva -Inf o NaN
        # en lugar de un LRA válido. Salimos temprano con 0.0 (rango neutro).
        if audio.size == 0 or float(np.max(np.abs(audio))) == 0.0:
            return 0.0
        data = _prepare_pyln_data(audio)
        num_samples = data.shape[0]
        # pyloudnorm.Meter.loudness_range requiere al menos un bloque de 3s (agrega 1.5s de silencio)
        if num_samples < int(sr * 1.5):
            return 0.0
        meter = pyln.Meter(sr)
        lra = meter.loudness_range(data)
        if np.isfinite(lra) and lra >= 0.0:
            return round(float(lra), 2)
        return 0.0
    except Exception:
        return 0.0
...
def _bandpass_filter(audio: np.ndarray, sr: int, lo: float, hi: float) -> np.ndarray:
    n_samples = audio.shape[-1] if audio.ndim >= 1 else 0
    if n_samples < 16:
...
def band_crest_and_stereo(audio: np.ndarray, sr: int, bands: list = DYNAMICS_BANDS) -> tuple:
    ...
    n_samples = audio.shape[-1] if audio.ndim >= 1 else 0
    if n_samples < 16:
        return {name: 0.0 for name, _, _ in bands}, {name: 1.0 for name, _, _ in bands}
...
def true_peak_dbfs(audio: np.ndarray, sr: int = 48000, oversample: int = 4) -> float:
    """Pico real (true peak) en dBTP con sobremuestreo polifásico 4x y recorte
    de transientes de frontera según ITU-R BS.1770-4 Anexo 2.
    Detecta de manera fiable inter-sample peaks que superan 0 dBTP.
    """
    ovs = max(1, int(oversample))
    if audio.ndim == 1:
        chans = [audio]
        ...
        if audio.shape[0] <= 8 and audio.shape[1] > audio.shape[0]:
            ...
            chans = [audio[:, i] for i in range(audio.shape[1])]
    else:
        chans = [audio.flatten()]
    ...
    for ch in chans:
        ...
        if len(ch) == 0:
            continue
        raw_peak = float(np.max(np.abs(ch)))
        peak = max(peak, raw_peak)
        if ovs > 1:
            up = resample_poly(ch, ovs, 1)
            # Recortar transientes de borde del filtro FIR
            trim = min(len(up) // 6, ovs * 10)
            if len(up) > 2 * trim and trim > 0:
                up_valid = up[trim:-trim]
            else:
                up_valid = up
            if len(up_valid) > 0:
                peak = max(peak, float(np.max(np.abs(up_valid))))
...
true_peak_dbtp = true_peak_dbfs


def measure_dc_offset(audio: np.ndarray) -> float:
    # F2.6: audio vacío devuelve NaN por np.mean([]); retornamos 0.0 (DC neutro)
    # y mantenemos el contrato de retornar float (nunca np.float64 / ndarray).
    if audio.size == 0:
        return 0.0
...
def evaluate_streaming_compliance(
    integrated_lufs: float,
    true_peak_dbtp: float,
    dynamic_range_db: float,
    lra: float,
) -> dict:
    """Evalúa el cumplimiento de loudness y picos frente a los perfiles de streaming
    principales (Spotify, Apple Music, YouTube) según especificaciones de plataforma y
    normas ITU-R BS.1770-4 / AES-TD1004.
    """
    ...
    platforms = {
        "spotify": {
            ...
            "gain_adjustment_db": -14.0 - integrated_lufs,
        },
        "apple_music": {
            "target_lufs": -16.0,
            "true_peak_ceiling_dbtp": -1.0,
            "gain_adjustment_db": -16.0 - integrated_lufs,
        },
        "youtube": {
            "target_lufs": -14.0,
            "true_peak_ceiling_dbtp": -1.0,
            "gain_adjustment_db": min(0.0, -14.0 - integrated_lufs),
        },
    }
    ...
    for name, cfg in platforms.items():
        ...
        ceiling = cfg["true_peak_ceiling_dbtp"]
        gain_adj = cfg["gain_adjustment_db"]
        projected_tp = true_peak_dbtp + gain_adj

        warnings = []
        # TRUE_PEAK_EXCEEDS_CEILING (when true_peak_dbtp > -1.0)
        if true_peak_dbtp > ceiling:
            warnings.append("TRUE_PEAK_EXCEEDS_CEILING")

        # CLIPPING_RISK (when true_peak_dbtp > 0.0 or projected true peak > 0.0 under gain boost)
        if true_peak_dbtp > 0.0 or (gain_adj > 0.0 and projected_tp > 0.0):
            warnings.append("CLIPPING_RISK")

        # EXCESSIVE_DYNAMIC_SQUASH (when dynamic_range_db < 6.0 or lra < 4.0)
        if dynamic_range_db < 6.0 or lra < 4.0:
            warnings.append("EXCESSIVE_DYNAMIC_SQUASH")

        # SIGNIFICANT_ATTENUATION (when integrated_lufs > target + 2.0 LU)
        if integrated_lufs > target + 2.0:
            warnings.append("SIGNIFICANT_ATTENUATION")

        # Determinar status: pass / warn / fail
        if "CLIPPING_RISK" in warnings:
            status = "fail"
        elif len(warnings) > 0:
            status = "warn"
        else:
        ...
        results[name] = {
            ...
            "gain_adjustment_db": round(float(gain_adj), 2),
            "projected_true_peak_dbtp": round(float(projected_tp), 2),
            "status": status,
            "warnings": warnings,
        }
...
evaluate_platform_compliance = evaluate_streaming_compliance


def normalize_to_streaming_target(
    audio: np.ndarray,
    sr: int,
    ...
    input_tp = true_peak_dbfs(work_audio, sr)

    n_samples = work_audio.shape[-1] if work_audio.ndim >= 1 else 0
    if not np.isfinite(input_lufs) or input_lufs <= -70.0 or n_samples < 16:
        info = {
            "platform": platform,
            "target_lufs": target_lufs,
            "ceiling_dbtp": ceiling_dbtp,
            "input_lufs": float(input_lufs),
            "input_true_peak_dbtp": float(input_tp),
            "output_lufs": float(input_lufs),
            "output_true_peak_dbtp": float(input_tp),
            "gain_applied_db": 0.0,
            "limited": False,
        }
        return (audio.copy(), info) if return_info else audio.copy()

    gain_db = float(target_lufs - input_lufs)
    gained = work_audio * (10.0 ** (gain_db / 20.0))

    # Medir True Peak post-ganancia
    projected_tp = true_peak_dbfs(gained, sr)
    ceiling_lin = float(10.0 ** (ceiling_dbtp / 20.0))
    was_limited = False

    if projected_tp > ceiling_dbtp + 0.01:
        # Aplicar brick-wall True Peak limiter linkeado
        limited = limiter(gained, sr, ceiling=ceiling_lin, oversample=4)
        post_tp = true_peak_dbfs(limited, sr)
        post_lufs = measure_lufs_integrated(limited, sr)

        # Ajuste fino si el limitador redujo levemente el loudness
        if post_lufs < target_lufs - 0.2 and post_tp < ceiling_dbtp - 0.05:
            fine_gain = min(target_lufs - post_lufs, ceiling_dbtp - post_tp)
            if fine_gain > 0.0:
                limited = limited * (10.0 ** (fine_gain / 20.0))
        out_audio = limited
        was_limited = True
    else:
        out_audio = gained

    final_out = out_audio.T if is_samples_first else out_audio
    final_lufs = measure_lufs_integrated(out_audio, sr)
    final_tp = true_peak_dbfs(out_audio, sr)

    info = {
        "platform": platform,
        "target_lufs": round(float(target_lufs), 2),
        "ceiling_dbtp": round(float(ceiling_dbtp), 2),
        "input_lufs": round(float(input_lufs), 2),
        "input_true_peak_dbtp": round(float(input_tp), 2),
        "output_lufs": round(float(final_lufs), 2),
        "output_true_peak_dbtp": round(float(final_tp), 2),
...
def analyze_audio(audio: np.ndarray, sr: int) -> dict:
    ...
    momentary_max, short_term_max = momentary_and_short_term_lufs(audio, sr)
    lra_val = measure_lra(audio, sr)
    platform_compliance = evaluate_streaming_compliance(
    ...
    st_loudness, _ = short_term_loudness_and_lra(audio, sr)
...
def compressor(audio: np.ndarray, sr: int,
    ...
    if ratio == 1.0:
        # F2.8: ratio=1.0 es bypass puro (sin compresión NI makeup). Devolvemos
        # copia exacta del audio para garantizar GR=0 y evitar cualquier
        # procesamiento numérico que pueda introducir artefactos/clicks.
        out = audio.copy()
        meter = {"gr_db": 0.0, "in_db": round(in_db, 2), "out_db": round(in_db, 2), "stereo_link": bool(stereo_link)}
...
def apply_mastering_chain(
    ...
    mb_pdr: bool = True,
    mb_pdr_hold_ms: float = 500.0,
    mb_bypass: bool = True,  # False = activo por defecto (bypass=False significa que la etapa SÍ corre)
    ...
    **_ignored,
    ...
    # ── 3b. Advanced DSP: Supresor espectral dinámico de resonancias (Soothe)
    if not adv_tamer_bypass:
        _chain_progress(14, "Aplicando supresor de resonancias (Soothe-style)")
        try:
            audio = dynamic_resonance_suppressor(
                audio, sr=sr,
                sensitivity=float(adv_tamer_sensitivity),
                depth_db=float(adv_tamer_depth_db),
                n_bands=int(adv_tamer_bands),
            )
        except Exception as _e_tamer:
            logger.warning("Error en dynamic_resonance_suppressor: %s", _e_tamer)
    ...
    # ── 10b. Advanced DSP: Maximizador de sonoridad polinomial (Inflator) ──
    if not adv_inflator_bypass and adv_inflator_drive > 0.0:
        _chain_progress(68, "Aplicando maximizador de sonoridad polinomial")
        try:
            audio = polynomial_inflator(
                audio,
                drive=float(adv_inflator_drive),
                mix=float(adv_inflator_mix),
                curve=str(adv_inflator_curve),
            )
        except Exception as _e_inf:
            logger.warning("Error en polynomial_inflator: %s", _e_inf)
    ...
    # ── 12b. Advanced DSP: Sintetizador de graves / fundamental fantasma ──
    if not adv_phantom_sub_bypass and adv_phantom_sub_mix > 0.0:
        _chain_progress(80, "Generando fundamental fantasma y sub-bass")
        _harm_map = {"octave": (2,), "fifth": (2, 3), "rich": (2, 3, 4)}
        _harmonics = _harm_map.get(str(adv_phantom_sub_mode).lower(), (2, 3))
        try:
            audio = phantom_sub_bass(
                audio, sr=sr,
                crossover_hz=float(adv_phantom_sub_crossover),
                harmonics=_harmonics,
                mix=float(adv_phantom_sub_mix),
            )
        except Exception as _e_sub:
            logger.warning("Error en phantom_sub_bass: %s", _e_sub)
    ...
    # ── 14b. Advanced DSP: Compensación adaptativa curvas isosónicas (ISO 226) ──
    if not adv_iso_bypass and adv_iso_strength > 0.0:
        _chain_progress(90, "Aplicando compensación isosónica ISO 226")
        try:
            audio = equal_loudness_compensation(
                audio, sr=sr,
                playback_phon=float(adv_iso_playback_phon),
                strength=float(adv_iso_strength),
            )
        except Exception as _e_iso:
            logger.warning("Error en equal_loudness_compensation: %s", _e_iso)
    ...
    chain_meters = {
        ...
        "adv_dsp": {
            "tamer": {"bypass": bool(adv_tamer_bypass), "sensitivity": float(adv_tamer_sensitivity), "depth_db": float(adv_tamer_depth_db)},
            "inflator": {"bypass": bool(adv_inflator_bypass), "drive": float(adv_inflator_drive), "mix": float(adv_inflator_mix), "curve": str(adv_inflator_curve)},
            "phantom_sub": {"bypass": bool(adv_phantom_sub_bypass), "crossover_hz": float(adv_phantom_sub_crossover), "mix": float(adv_phantom_sub_mix)},
            "iso": {"bypass": bool(adv_iso_bypass), "playback_phon": float(adv_iso_playback_phon), "strength": float(adv_iso_strength)},
        },
...
def run_lufs_safety_check(audio_base: np.ndarray, sr: int, chain_kwargs: dict,
    ...
    original_input_gain = current_input_gain
    ...
    for i in range(max_iters):
        ...
        if abs(delta) <= tolerance_db:
            ...
            break
        new_input_gain = float(np.clip(current_input_gain + delta, max(-24.0, original_input_gain), 24.0))
...
def process_audio(
    ...
    if preview_seconds is not None:
        audio = _crop_preview(audio, sr, preview_seconds, start_sec=preview_start_sec)
    ...
    _chain_params = MasteringParams(
        ...
        adv_tamer_bypass=adv_tamer_bypass, adv_tamer_sensitivity=adv_tamer_sensitivity,
        adv_tamer_depth_db=adv_tamer_depth_db, adv_tamer_bands=adv_tamer_bands,
        adv_inflator_bypass=adv_inflator_bypass, adv_inflator_drive=adv_inflator_drive,
        adv_inflator_mix=adv_inflator_mix, adv_inflator_curve=adv_inflator_curve,
        adv_phantom_sub_bypass=adv_phantom_sub_bypass, adv_phantom_sub_crossover=adv_phantom_sub_crossover,
        adv_phantom_sub_mix=adv_phantom_sub_mix, adv_phantom_sub_mode=adv_phantom_sub_mode,
        adv_iso_bypass=adv_iso_bypass, adv_iso_playback_phon=adv_iso_playback_phon,
        adv_iso_strength=adv_iso_strength,
    )
    ...
    # Meters timeline para display real-time (100% server-side).
    # Solo para preview (25s) — no vale la pena para master full track.
    if preview_seconds is not None:
        try:
            _report(progress_cb, 96, "Computando timeline de medidores")
            chain_meters["meters_timeline"] = compute_meters_timeline(audio, sr, block_ms=33.0)
        except Exception as _tl_err:
            logger.warning("compute_meters_timeline failed: %s", _tl_err) if logger else None
...
# ─────────────────────────────────────────────────────────────────────────────
# B1.2 — Phase rotation (all-pass variable 0-360°) — Sprint 4
# ─────────────────────────────────────────────────────────────────────────────

def _design_phase_shift_ap(omega_d: float, phi_target_rad: float) -> tuple:
    """Diseña un all-pass de 2º orden que rota la fase en exactamente
    ``phi_target_rad`` a la frecuencia digital ``omega_d`` (rad/muestra).

    Estrategia: buscar los coeficientes del AP (polo conjugado en ``r·e^{±jθ}``,
    ceros en el recíproco conjugado) minimizando la diferencia entre la fase
    del AP en ``omega_d`` y el target, con la restricción dura ``|H(ω_d)|=1``
    (propiedad AP se cumple por construcción simétrica polo/zero).

    Devuelve ``(b, a)`` para ``H(z) = (b0 + b1·z⁻¹ + b2·z⁻²) / (1 + a1·z⁻¹ + a2·z⁻²)``,
    o ``None`` si la optimización no converge.

    Razón de usar scipy.optimize: una fórmula cerrada para AP de 2º orden
    con fase arbitraria en una frecuencia específica no existe (los APs
    simétricos polo/zero tienen fase acotada a ~±180° y punto de wrap);
    la búsqueda numérica es robusta y se cachea implícitamente porque
    el resultado solo depende de ``(omega_d, phi_target)``.
    """
    def _cost(params):
        r, theta = float(params[0]), float(params[1])
        if r <= 1e-3 or r >= 0.999:
            return 1e6
        p = r * np.exp(1j * theta)
        # Ceros en el RECÍPROCO de los polos (no conjugado): z_i = 1/p_i.
        # Para polos conjugados p=r·e^{±jθ}, los ceros son (1/r)·e^{±jθ}
        # con el ángulo INVERTIDO respecto al polo. Esto es lo que hace que
        # ``(a + b·z⁻¹ + z⁻²)/(1 + b·z⁻¹ + a·z⁻²)`` sea AP canónico.
        zeros = [1.0 / p, 1.0 / p.conjugate()]
        poles = [p, p.conjugate()]
        try:
            sos = _zpk2sos(zeros, poles, 1.0)
            b_loc = sos[0, :3]   # polinomio del numerador (en forma monic)
            a_loc = sos[0, 3:]   # polinomio del denominador
            # Re-escalar numerador: el AP canónico es (a·z² + b·z + 1)/(z² + b·z + a)
            # con a = r², mientras ``np.poly`` lo devuelve en forma monic (z² + ...).
            # Sin esta escala, |H| = 1/r² en vez de 1.
            b_loc = b_loc * (r * r)
            _, h = _freqz(b_loc, a_loc, worN=[float(omega_d)])
            mag = float(np.abs(h[0]))
            phase = float(np.angle(h[0]))
            if not np.isfinite(mag) or not np.isfinite(phase):
                return 1e6
            # Diferencia de fase con manejo de wrap (±π): restamos múltiplos de 2π
            dphi = phase - phi_target_rad
            dphi = ((dphi + np.pi) % (2.0 * np.pi)) - np.pi
            return float(dphi * dphi + (mag - 1.0) * (mag - 1.0))
        except Exception:
            return 1e6

    best, best_cost = None, float("inf")
    for r0 in (0.7, 0.85, 0.95, 0.99):
        for theta0 in (
            float(omega_d) - 0.30,
            float(omega_d) - 0.10,
            float(omega_d),
            ...
            try:
                res = _sp_minimize(_cost, [r0, theta0], method="Nelder-Mead",
            ...
            if res.fun < best_cost:
                ...
                best = res.x
    if best is None or best_cost > 1e-3:
        return None
    r, theta = float(best[0]), float(best[1])
    p = r * np.exp(1j * theta)
    zeros = [1.0 / p, 1.0 / p.conjugate()]
    poles = [p, p.conjugate()]
    sos = _zpk2sos(zeros, poles, 1.0)
    b_final = sos[0, :3].copy() * (r * r)  # re-escalar (ver _cost para justificación)
    a_final = sos[0, 3:].copy()
    return b_final, a_final


def phase_rotation(audio: np.ndarray, sr: int, freq_hz: float,
                   angle_deg: float, q: float = 1.0) -> np.ndarray:
    """All-pass variable para rotación de fase en una banda centrada en
    ...
    Pipeline:
        ...
        3) Sumar al audio original con patrón dry + (wet - dry) (paralelo),
           preservando intacto lo que está fuera de la banda.
        4) Normalizar pico si pasa de 1.0 para evitar clipping.

    Args:
        audio: ``[N, 2]`` (estéreo) o ``[N]`` (mono), rango [-1, 1] float.
        sr: sample rate en Hz.
        freq_hz: frecuencia central de la rotación (Hz).
        angle_deg: rotación de fase en grados; se acepta el rango completo
            0-360° y se normaliza a (-180°, 180°] para el AP (un AP
            simétrico polo/zero solo cubre ese rango; 270° ≡ -90°).
        q: factor Q que controla el ancho de banda del Butterworth. Convención:
            ``bandwidth_hz = freq_hz / q``. Q=1 → ancho = freq_hz (default).

    Returns:
        ``np.ndarray`` con la misma forma/dtype que ``audio`` (float32),
        con la fase de la banda rotada. ``audio`` no se muta.
    """
    if audio.size == 0:
    ...
    # Acotar freq_hz y Q a rangos sensatos.
    nyq = float(sr) / 2.0
    freq_hz = float(np.clip(freq_hz, 5.0, nyq - 5.0))
    q = float(np.clip(q, 0.1, 30.0))

    # Normalizar ángulo a (-180°, 180°] — el AP simétrico polo/zero solo cubre ese
    # rango; fuera de él la rotación es equivalente (270° = -90° + 360°).
    angle_norm = float(angle_deg) % 360.0
    if angle_norm > 180.0:
        angle_norm -= 360.0
    ...
    omega_d = 2.0 * np.pi * freq_hz / float(sr)
    phi_target_rad = float(np.deg2rad(angle_norm))
    ba = _design_phase_shift_ap(omega_d, phi_target_rad)

    if ba is None:
        # Fallback rarísimo (optim no converge): devolver dry sin cambios.
        logger.warning(
            "phase_rotation: optimizer no convergió para freq=%.1fHz angle=%.1f°; "
            "se devuelve audio sin modificar.", freq_hz, angle_deg,
        )
        return np.array(audio, copy=True, dtype=np.float32)
    ...
    try:
        if band_sig.ndim == 1:
            band_shifted = _lfilter(b_ap, a_ap, band_sig).astype(np.float32, copy=False)
        else:
            band_shifted = np.stack(
                [_lfilter(b_ap, a_ap, ch).astype(np.float32, copy=False) for ch in band_sig]
            )
    except Exception as exc:
        logger.warning("phase_rotation: lfilter falló (%s); se devuelve audio sin modificar.", exc)
        return np.array(audio, copy=True, dtype=np.float32)
...
def process_audio_with_multi_reference(
    input_path: str,
    reference_paths: list,
    reference_weights: list = None,
    **kwargs,
) -> dict:
    """Mastering por referencia con múltiples referencias comerciales.

    Args:
        input_path: path al archivo de entrada.
        reference_paths: lista de paths a archivos de referencia (2-5).
        reference_weights: pesos relativos de cada referencia (se normalizan).
        **kwargs: pasados a process_audio_with_reference.

    Devuelve el mismo dict que process_audio_with_reference, más info del blend.
    """
    if not reference_paths or len(reference_paths) < 1:
        raise ValueError("Se necesita al menos una referencia.")

    if len(reference_paths) == 1:
        return process_audio_with_reference(input_path, reference_paths[0], **kwargs)

    weights = reference_weights or [1.0] * len(reference_paths)
    if len(weights) != len(reference_paths):
        weights = [1.0] * len(reference_paths)
    total_w = sum(float(w) for w in weights)
    if total_w <= 0:
        weights = [1.0 / len(reference_paths)] * len(reference_paths)
    else:
    ...
    for rp in reference_paths:
        ref_audio, ref_sr = librosa.load(rp, sr=None, mono=False)
        if ref_audio.ndim == 1:
    ...
    target_sr = ref_srs[0]
    for i, (ra, rs) in enumerate(zip(ref_audios, ref_srs)):
        if rs != target_sr:
            ref_audios[i] = librosa.resample(ra, orig_sr=rs, target_sr=target_sr)

    min_len = min(ra.shape[-1] for ra in ref_audios)
    mixed_ref = np.zeros((2, min_len), dtype=np.float32)
    for ra, w in zip(ref_audios, weights):
        ra_trim = ra[..., :min_len]
        if ra_trim.shape[0] == 1:
            ra_trim = np.vstack([ra_trim, ra_trim])
        mixed_ref += (ra_trim * w).astype(np.float32)

    peak = np.max(np.abs(mixed_ref))
    if peak > 0.99:
        mixed_ref = mixed_ref / peak

    import tempfile
    import soundfile as sf
    tmp_ref = tempfile.NamedTemporaryFile(suffix=".wav", delete=False, dir=os.path.dirname(input_path))
    sf.write(tmp_ref.name, mixed_ref.T, target_sr, subtype="PCM_24")

    try:
        result = process_audio_with_reference(input_path, tmp_ref.name, **kwargs)
        result["multi_reference"] = {
            "count": len(reference_paths),
            "weights": [round(w, 3) for w in weights],
            "reference_srs": ref_srs,
            "blended_sr": target_sr,
        }
        return result
    finally:
